...
def test_divide_scalar_forward():
    np.testing.assert_allclose(
        ad.ops.divide_scalar(ad.Tensor([[1.7, 1.45]]), scalar=12).numpy(),
...
def test_matmul_forward():
    np.testing.assert_allclose(
        ad.ops.matmul(
            ad.Tensor([[4.95, 1.75, 0.25], [4.15, 4.25, 0.3], [0.3, 0.4, 2.1]]),
            ad.Tensor([[1.35, 2.2, 1.55], [3.85, 4.8, 2.6], [1.15, 0.85, 4.15]]),
        ).numpy(),
        np.array(
    ...
    np.testing.assert_allclose(
        ad.ops.matmul(
            ad.Tensor([[3.8, 0.05], [2.3, 3.35], [1.6, 2.6]]),
            ad.Tensor([[1.1, 3.5, 3.7], [0.05, 1.25, 1.0]]),
        ).numpy(),
        np.array(
    ...
    np.testing.assert_allclose(
        ad.ops.matmul(
            ad.Tensor([[1.9, 1.9], [4.8, 4.9], [3.25, 3.75]]),
            ad.Tensor(
                [
    ...
    np.testing.assert_allclose(
        ad.ops.matmul(
            ...
            ad.Tensor([[2.2, 0.65, 2.5], [2.5, 1.3, 0.15]]),
        ).numpy(),
        np.array(
            [
                [
...
def test_summation_forward():
    ...
    np.testing.assert_allclose(
        ad.ops.summation(
            ad.Tensor([[1.5, 3.85, 3.45], [1.35, 1.3, 0.65], [2.6, 4.55, 0.25]]), axis=0
        ).numpy(),
...
def test_broadcast_to_forward():
    np.testing.assert_allclose(
        ad.ops.broadcast_to(ad.Tensor([[1.85, 0.85, 0.6]]), shape=(3, 3, 3)).numpy(),
        np.array(
            [
...
def test_negate_forward():
    np.testing.assert_allclose(
        ad.ops.negate(ad.Tensor([[1.45, 0.55]])).numpy(), np.array([[-1.45, -0.55]])
    )


def test_transpose_forward():
    np.testing.assert_allclose(
        ad.ops.transpose(ad.Tensor([[[1.95]], [[2.7]], [[3.75]]]), axis=(1, 2)).numpy(),
        np.array([[[1.95]], [[2.7]], [[3.75]]]),
    )
    np.testing.assert_allclose(
        ad.ops.transpose(
            ad.Tensor([[[[0.95]]], [[[2.55]]], [[[0.45]]]]), axis=(2, 3)
        ).numpy(),
    ...
    np.testing.assert_allclose(
        ad.ops.transpose(ad.Tensor([[[2.45]], [[3.5]], [[0.9]]]), axis=(0, 1)).numpy(),
        np.array([[[2.45], [3.5], [0.9]]]),
    )
    np.testing.assert_allclose(
        ad.ops.transpose(ad.Tensor([[4.4, 2.05], [1.85, 2.25], [0.15, 1.4]])).numpy(),
        np.array([[4.4, 1.85, 0.15], [2.05, 2.25, 1.4]]),
    )
    np.testing.assert_allclose(
        ad.ops.transpose(
            ad.Tensor([[0.05, 3.7, 1.35], [4.45, 3.25, 1.95], [2.45, 4.4, 4.5]])
        ).numpy(),
...
def gradient_check(f, *args, tol=1e-6, backward=False, **kwargs):
    ...
    for i in range(len(args)):
        for j in range(args[i].realized_cached_data().size):
            args[i].realized_cached_data().flat[j] += eps
            f1 = float(f(*args, **kwargs).numpy().sum())
            args[i].realized_cached_data().flat[j] -= 2 * eps
    ...
    if not backward:
        ...
        computed_grads = [
            x.numpy()
            for x in out.op.gradient_as_tuple(ad.Tensor(np.ones(out.shape)), out)
        ]
    else:
...
def test_divide_backward():
    gradient_check(
        ad.ops.divide,
        ad.Tensor(np.random.randn(5, 4)),
        ad.Tensor(5 + np.random.randn(5, 4)),
    )


def test_divide_scalar_backward():
    gradient_check(
        ad.ops.divide_scalar, ad.Tensor(np.random.randn(5, 4)), scalar=np.random.randn(1)
    )


def test_matmul_simple_backward():
    gradient_check(
        ad.ops.matmul, ad.Tensor(np.random.randn(5, 4)), ad.Tensor(np.random.randn(4, 5))
    )
...
def test_matmul_batched_backward():
    gradient_check(
        ad.ops.matmul,
        ad.Tensor(np.random.randn(6, 6, 5, 4)),
        ad.Tensor(np.random.randn(6, 6, 4, 3)),
    )
    gradient_check(
        ad.ops.matmul,
        ad.Tensor(np.random.randn(6, 6, 5, 4)),
        ad.Tensor(np.random.randn(4, 3)),
    )
    gradient_check(
        ad.ops.matmul,
        ad.Tensor(np.random.randn(5, 4)),
...
def test_reshape_backward():
    gradient_check(ad.ops.reshape, ad.Tensor(np.random.randn(5, 4)), shape=(4, 5))


def test_negate_backward():
    gradient_check(ad.ops.negate, ad.Tensor(np.random.randn(5, 4)))


def test_transpose_backward():
    gradient_check(ad.ops.transpose, ad.Tensor(np.random.randn(3, 5, 4)), axis=(1, 2))
    gradient_check(ad.ops.transpose, ad.Tensor(np.random.randn(3, 5, 4)), axis=(0, 1))


def test_broadcast_to_backward():
    gradient_check(ad.ops.broadcast_to, ad.Tensor(np.random.randn(3, 1)), shape=(3, 3))
    gradient_check(ad.ops.broadcast_to, ad.Tensor(np.random.randn(1, 3)), shape=(3, 3))
    gradient_check(
        ad.ops.broadcast_to,
        ad.Tensor(
            np.random.randn(
    ...
    gradient_check(ad.ops.broadcast_to, ad.Tensor(np.random.randn()), shape=(3, 3, 3))
    gradient_check(
        ad.ops.broadcast_to, ad.Tensor(np.random.randn(5, 4, 1)), shape=(5, 4, 3)
    )


def test_summation_backward():
    gradient_check(ad.ops.summation, ad.Tensor(np.random.randn(5, 4)), axis=(1,))
    gradient_check(ad.ops.summation, ad.Tensor(np.random.randn(5, 4)), axis=(0,))
    gradient_check(ad.ops.summation, ad.Tensor(np.random.randn(5, 4)), axis=(0, 1))
    gradient_check(ad.ops.summation, ad.Tensor(np.random.randn(5, 4, 1)), axis=(0, 1))
...
def test_topo_sort():
    # Test case 1
    a1, b1 = ad.Tensor(np.asarray([[0.88282157]])), ad.Tensor(
    ...
    topo_order = np.array([x.numpy() for x in ad.find_topo_sort([c1])])

    assert len(soln) == len(topo_order)
    np.testing.assert_allclose(topo_order, soln, rtol=1e-06, atol=1e-06)

    # Test case 2
    a1, b1 = ad.Tensor(np.asarray([[0.20914675], [0.65264178]])), ad.Tensor(
    ...
    topo_order = [x.numpy() for x in ad.autograd.find_topo_sort([c1])]
    ...
    # Test case 3
    a = ad.Tensor(np.asarray([[1.4335016, 0.30559972], [0.08130171, -1.15072371]]))
    b = ad.Tensor(np.asarray([[1.34571691, -0.95584433], [-0.99428573, -0.04017499]]))
    e = (a @ b + b - a) @ a

    topo_order = np.array([x.numpy() for x in ad.autograd.find_topo_sort([e])])
...
def test_compute_gradient():
    gradient_check(
        lambda A, B, C: ad.ops.summation((A @ B + C) * (A @ B), axis=None),
        ad.Tensor(np.random.randn(10, 9)),
        ad.Tensor(np.random.randn(9, 8)),
        ad.Tensor(np.random.randn(10, 8)),
        backward=True,
    )
    gradient_check(
        lambda A, B: ad.ops.summation(ad.ops.broadcast_to(A, shape=(10, 9)) * B, axis=None),
        ad.Tensor(np.random.randn(10, 1)),
        ad.Tensor(np.random.randn(10, 9)),
        backward=True,
    )
    gradient_check(
        lambda A, B, C: ad.ops.summation(
            ad.ops.reshape(A, shape=(10, 10)) @ B / 5 + C, axis=None
        ),
        ad.Tensor(np.random.randn(100)),
        ad.Tensor(np.random.randn(10, 5)),
        ad.Tensor(np.random.randn(10, 5)),
        backward=True,
    )
